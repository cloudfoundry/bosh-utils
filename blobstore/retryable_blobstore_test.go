	...
	Describe("Get", func() {
		Context("when inner blobstore succeeds before maximum number of get tries (first time)", func() {
			It("returns path without an error", func() {
				...
				path, err := retryableBlobstore.Get("fake-blob-id", boshcrypto.MustNewMultipleDigest(boshcrypto.NewDigest(boshcrypto.DigestAlgorithmSHA1, "fingerprint")))
				...
				Expect(innerBlobstore.GetFingerprints[0]).To(Equal(boshcrypto.MustNewMultipleDigest(boshcrypto.NewDigest(boshcrypto.DigestAlgorithmSHA1, "fingerprint"))))
			})
		})
		...
		Context("when inner blobstore succeed exactly at maximum number of get tries", func() {
			It("returns path without an error", func() {
				...
				Expect(innerBlobstore.GetFingerprints).To(ConsistOf(
					boshcrypto.MustNewMultipleDigest(
						boshcrypto.NewDigest(boshcrypto.DigestAlgorithmSHA1, "fingerprint"),
					),
					boshcrypto.MustNewMultipleDigest(
						boshcrypto.NewDigest(boshcrypto.DigestAlgorithmSHA1, "fingerprint"),
					),
		...
		Context("when inner blobstore does not succeed before maximum number of get tries", func() {
			It("returns last try error from inner blobstore", func() {
				...
				_, err := retryableBlobstore.Get("fake-blob-id", boshcrypto.MustNewMultipleDigest(boshcrypto.NewDigest(boshcrypto.DigestAlgorithmSHA1, "fingerprint")))
