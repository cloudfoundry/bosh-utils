...
var _ = Describe("localBlobstore", func() {
	...
	Describe("Get", func() {
		It("fetches the local blob contents", func() {
			...
			_, err = blobstore.Get("fake-blob-id", crypto.MustNewMultipleDigest(crypto.NewDigest(crypto.DigestAlgorithmSHA1, "sha-sum")))
			Expect(err).ToNot(HaveOccurred())
		...
		It("errs when temp file create errs", func() {
			...
			fileName, err := blobstore.Get("fake-blob-id", crypto.MustNewMultipleDigest(crypto.NewDigest(crypto.DigestAlgorithmSHA1, "sha-sum")))
			Expect(err).To(HaveOccurred())
	...
	Describe("Delete", func() {
		It("removes the blob from the blobstore", func() {
			...
			_, err = blobstore.Get(blobID, crypto.MustNewMultipleDigest(crypto.NewDigest(crypto.DigestAlgorithmSHA1, "sha-sum")))
			Expect(err).ToNot(HaveOccurred())
